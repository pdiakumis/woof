...
write.table(as.data.frame(t(purity_ploidy)), paste0(base_nm, "_purply.tsv"), row.names = FALSE, sep = "\t", quote = FALSE)

# export cncf
cncf <- facets_list[["cncf"]]
write.table(cncf, paste0(base_nm, "_segs.tsv"), row.names = FALSE, sep = "\t", quote = FALSE)
