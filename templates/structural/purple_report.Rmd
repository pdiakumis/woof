...
params <- list(tumor_name = "PRJ180580_P055_Cells", 
              results_dir = "/Users/pdiakumis/Desktop/projects/umccr/patients/P055/purple")
render_me <- function() {
  rmarkdown::render("purple_report.Rmd",
                    output_file = file.path(params$results_dir, paste0(params$tumor_name, "_purple_report.html")),
                    params = params)
}
...
az300_genes <- readr::read_tsv(
  system.file("extdata", "az300.bed", package = "pebbles"), 
  col_names = c("chrom", "start", "end", "gene"), col_types = "ciic") %>% 
  dplyr::pull(gene)

umccr_genes <- readr::read_tsv("~/Desktop/projects/umccr/woof/templates/structural/cancer_genes.txt", col_names = "gene", col_types = "c") %>% 
  dplyr::pull(gene)

all_genes <- union(az300_genes, umccr_genes)
  

df %>% 
  filter(gene %in% c(az300_genes, umccr_genes)) %>% 
...
cnv_germ <- readr::read_tsv(fn_germ, col_types = "ciiddddcccdddd") %>%
