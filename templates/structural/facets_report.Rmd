...
require(devtools)
require(DT)
require(facets) # just for version info
```
...
cnv_tab <- function(cnv) {
  ...
  d <- DT::datatable(cnv, filter = "top", rownames = FALSE)
  d <- formatCurrency(d, 'start', currency = "", interval = 3, mark = ",", digits = 0)
  d <- formatCurrency(d, 'end', currency = "", interval = 3, mark = ",", digits = 0)
  d
}
```
