...
config['tools']['vcfvalidator']['run'] = {
    'outdir' : join(config['tools']['vcfvalidator']['outdir'], 'validate')
}
...
rule vcfvalidator_run:
    ...
    log:
        log = join(config['woof']['final_dir'], 'logs', '{batch}', '{fname}_vcfvalidator_validate.log')
    shell:
