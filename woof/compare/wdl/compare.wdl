...
workflow compare_vcf_files {
  ...
  input {
    # sample [0], ftype [1], run1 [2], run2 [3]
  ...
  scatter (sample in inputSamples) {
    ...
    call count.count_vars as c_all_f1 { input: vcf = sample[2], outdir = outdir + sample[0] + "/vcf_counts/f1/" + sample[1] + "/ALL/" }
    call count.count_vars as c_all_f2 { input: vcf = sample[3], outdir = outdir + sample[0] + "/vcf_counts/f2/" + sample[1] + "/ALL/"}
    call count.count_vars as c_pass_f1 { input: vcf = filter_f1.out, outdir = outdir + sample[0] + "/vcf_counts/f1/" + sample[1] + "/PASS/" }
    call count.count_vars as c_pass_f2 { input: vcf = filter_f2.out, outdir = outdir + sample[0] + "/vcf_counts/f2/" + sample[1] + "/PASS/" }
